...
def prepare_data(data_mat, data_path_src, save_dir, body_image_size, dataset_type='train', generate_npy=False, debug_mode=False):
  '''
    ...
    for person in range(nop):
      ...
      try:
        ...
        if not os.path.exists(image_path):
          ...
          body_cv = cv2.resize(body, (body_image_size,body_image_size))
...
def parse_args():
    ...
    parser.add_argument('--body_image_size', type=int, default=128)
    # Generate args
...
if __name__ == '__main__':
    ...
    for label in labels:
      ...
      prepare_data(data_mat, data_path_src, save_path, body_image_size=args.body_image_size, dataset_type=label, generate_npy=args.generate_npy, debug_mode=args.debug_mode)
